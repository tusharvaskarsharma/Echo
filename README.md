# Emmy Ã¢â‚¬â€ AI-Powered Living Legacy Platform

> *An AI-powered living legacy system that preserves how a person thinks, speaks, and loves Ã¢â‚¬â€ so the people who matter most never lose them.*

**Hackathon:** OpenAI Hackathon (Devpost)
**Stack:** Gemini Live Audio Ã‚Â· Groq persona generation Ã‚Â· Next.js 14 Ã‚Â· FastAPI Ã‚Â· Supabase Ã‚Â· Pinecone Ã‚Â· librosa Ã‚Â· CREPE Pitch Tracking Ã‚Â· ElevenLabs Voice Clone Ã‚Â· Acoustic Fingerprint Engine
...
4. [Step-by-Step Building Process & Data Flow](#4-step-by-step-building-process--data-flow)
   - [Phase 1 Ã¢â‚¬â€ Data Collection & Ingestion: The Interview Session Pipeline](#phase-1--data-collection--ingestion-the-interview-session-pipeline)
   - [Phase 2 Ã¢â‚¬â€ Backend & AI Logic: The Family Conversation Pipeline](#phase-2--backend--ai-logic-the-family-conversation-pipeline)
   - [Phase 3 Ã¢â‚¬â€ Frontend Integration: Building the UI/UX](#phase-3--frontend-integration-building-the-uiux)
...
6. [Personalised Voice Fingerprint Engine (Future Roadmap)](#6-personalised-voice-fingerprint-engine-future-roadmap)
   ...
   - [The Technical Pipeline Ã¢â‚¬â€ End to End](#62-the-technical-pipeline--end-to-end)
...
**Emmy** is a consent-first, multimodal AI platform that transforms structured life-narrative sessions into a deeply personal, privately hosted "voice model" of a human being. Unlike a memorial website or a static recording, Emmy enables family members to have *new* conversations with a loved one's preserved memory Ã¢â‚¬â€ conversations grounded exclusively in what that person actually shared, recalled in their voice, reasoning style, and emotional cadence.

> **Core Value Proposition:** Emmy uses Gemini Live voice sessions, Groq-powered retrieval-grounded persona responses, and a consent-governed memory graph to let families continue meaningful conversations with the mind of someone they've lost Ã¢â‚¬â€ not a simulation, but a reflection.

**Critical ethical differentiator:** Emmy never fabricates. If a grandmother never shared a memory about a topic, her Emmy responds: *"I don't have a memory of that Ã¢â‚¬â€ I wish I did."* This boundary is technically enforced through RAG-only generation with a strict no-hallucination system prompt, making Emmy trustworthy rather than uncanny.

---

### 1.2 Market Validity: Target User Personas

#### Primary Persona Ã¢â‚¬â€ The Subject

**Demographics:**
- Age 55Ã¢â‚¬â€œ80, often retired or semi-retired
- Has grandchildren or adult children who live remotely
- May have received an early-stage diagnosis (dementia, cancer)
- Moderate tech comfort Ã¢â‚¬â€ uses smartphone daily
...
#### Secondary Persona Ã¢â‚¬â€ The Family Member

- Adult child, age 30Ã¢â‚¬â€œ55, often geographically distant
...
#### Tertiary Persona Ã¢â‚¬â€ The Proactive Planner

- Age 35Ã¢â‚¬â€œ55, any health status
- Has watched a parent die without documenting their story
- Motivated by not repeating that loss for their own children
- Uses tools like life insurance, wills Ã¢â‚¬â€ treats this as legacy infrastructure
...
3. **Cultural moment** Ã¢â‚¬â€ post-COVID emphasis on digital legacy, combined with growing distrust of "AI ghost" companies that do this without consent, creates an opening for an ethical, subject-first competitor.

> **The competitive gap:** Existing players (HereAfter AI, StoryFile, Eternos) collect video recordings and build chatbots from transcripts. None use live audio AI for collection, none offer fine-tuned persona models, and none have a robust consent architecture. Emmy is not a better version of these products Ã¢â‚¬â€ it is a fundamentally different category.
...
- **Dual-database layer** Ã¢â‚¬â€ PostgreSQL via Supabase for structured data, Pinecone for vector embeddings
- **Multi-model AI layer** Ã¢â‚¬â€ Gemini Live for native session audio and Gemini embeddings; Groq Llama for persona generation, structured extraction, and audio transcription
- **Mind Model layer** Ã¢â‚¬â€ a consent-gated, evidence-linked cognitive model of values, reasoning, emotions, communication, and evolving life principles. It augments retrieval; it never substitutes unsupported traits for source memories.
- **Cognitive Engine layer** Ã¢â‚¬â€ an intent-aware, relationship- and time-sensitive answer-planning layer that creates a bounded evidence ledger before persona generation. It never stores or exposes chain-of-thought.
- **Autonomous Life Interview Engine** Ã¢â‚¬â€ a voluntary, coverage-aware long-horizon planner that identifies unexplored domains, recommends respectful follow-ups, tracks uncertainty, and keeps the subject in control.
...
| UI Components | shadcn/ui + Tailwind CSS | shadcn's copy-paste model means zero install overhead for accessible components (dialogs, toasts, sliders). Tailwind prevents style conflicts. The unstyled foundation allows the warm, human aesthetic Emmy requires. |
...
| Primary Database | PostgreSQL via Supabase | Supabase provides row-level security (RLS) out of the box Ã¢â‚¬â€ essential for Emmy's consent architecture where subjects control exactly which rows are accessible to which family members. Realtime subscriptions enable live UI updates during session processing. |
...
```
emmy/                                    # Monorepo root
Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ .env.example                         # All required env vars documented
Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ docker-compose.yml                   # Local: postgres and pgadmin
Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ turbo.json                           # Turborepo pipeline config
Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ package.json                         # Workspace root Ã¢â‚¬â€ no direct deps
Ã¢â€â€š
Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ apps/
Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ web/                             # Next.js 14 frontend
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ next.config.ts               # Experimental: serverComponentsExternalPackages
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ tailwind.config.ts           # Emmy design tokens (warm palette, radius system)
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ middleware.ts                # Supabase auth middleware Ã¢â‚¬â€ protects all /app routes
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ app/                         # App Router root
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ (auth)/
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ login/
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ page.tsx         # Magic link + SMS OTP login
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ callback/
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š       Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ route.ts         # Supabase auth callback handler
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ (subject)/               # Routes for the person being recorded
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ dashboard/
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ page.tsx         # Memory map overview, session history
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ session/
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ page.tsx         # Interview session entry point
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ layout.tsx       # Minimal chrome Ã¢â‚¬â€ no distractions during session
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ memories/
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ page.tsx         # Visual memory graph with consent controls
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ [id]/page.tsx    # Individual memory review + edit + privacy toggle
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ legacy/
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š       Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ page.tsx         # Manage legacy contacts, access levels, revocation
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ (family)/                # Routes for approved family members
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ invite/[token]/
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ page.tsx         # Accept legacy invitation Ã¢â‚¬â€ consent acknowledgment UI
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ conversation/[emmyId]/
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š       Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ page.tsx         # The family conversation interface (voice + text)
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ api/                     # Next.js API routes (thin proxies to FastAPI)
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š       Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ session/
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ token/route.ts   # Mint ephemeral Realtime API token (server-side)
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š       Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ audio/
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š           Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ playback/route.ts# Proxy TTS audio stream to client
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ components/
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ session/
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ AudioOrb.tsx         # Animated orb that pulses with audio amplitude
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ TranscriptStream.tsx # Real-time transcript with emotion badges
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ SessionControls.tsx  # Pause/resume/end session controls
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ MemoryFlash.tsx      # Pop-up when a new memory is tagged (real-time)
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ memory/
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ MemoryGraph.tsx      # D3-powered force graph of memory nodes
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ MemoryCard.tsx       # Individual memory tile with privacy toggle
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ ConsentSlider.tsx    # Three-state (private/family/public) toggle
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ conversation/
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ VoiceInput.tsx       # Family member speaks Ã¢â‚¬â€ WebRTC capture
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ EmmyResponse.tsx     # Audio playback of Emmy's response with waveform
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ ConversationLog.tsx  # Running transcript of the family conversation
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ ui/                      # shadcn/ui primitives (Button, Dialog, etc.)
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ hooks/
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ useRealtimeSession.ts    # Gemini Live WebSocket lifecycle
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ useAudioAmplitude.ts     # Web Audio API amplitude for orb animation
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ useMemoryGraph.ts        # SWR fetcher + real-time Supabase subscription
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€š
Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ lib/
Ã¢â€â€š   Ã¢â€â€š       Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ supabase/client.ts       # Browser Supabase client (anon key)
Ã¢â€â€š   Ã¢â€â€š       Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ supabase/server.ts       # Server-side Supabase client (service role)
Ã¢â€â€š   Ã¢â€â€š       Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ api.ts                   # Typed FastAPI client (from openapi-typescript)
Ã¢â€â€š   Ã¢â€â€š
Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ api/                             # FastAPI backend
Ã¢â€â€š       Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ pyproject.toml               # FastAPI, Supabase, Pinecone, and provider-neutral HTTP deps
Ã¢â€â€š       Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Dockerfile                   # Multi-stage: builder (uv install) Ã¢â€ â€™ runtime
Ã¢â€â€š       Ã¢â€â€š
Ã¢â€â€š       Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ app/
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ main.py                  # FastAPI app factory, CORS, lifespan context
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ config.py                # Pydantic Settings Ã¢â‚¬â€ all env vars with validation
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ routers/
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ sessions.py          # POST /sessions, GET /sessions/{id}, PATCH status
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ memories.py          # CRUD for memory fragments + consent updates
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ emmy.py              # POST /emmy/converse Ã¢â‚¬â€ the family conversation endpoint
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ finetune.py          # Trigger + poll fine-tune jobs
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ services/
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ memory_extractor.py  # Groq JSON output Ã¢â€ â€™ MemoryFragment schema
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ embedding_service.py # Batch embed + upsert to Pinecone
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ retrieval_service.py # Consent-aware semantic retrieval from Pinecone
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ persona_service.py   # Query fine-tuned model + build response
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ finetune_builder.py  # Construct JSONL training file from memory graph
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ workers/                 # Synchronous processing modules
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ process_session.py   # Full post-session pipeline
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ retrain_persona.py   # Incremental fine-tune after new sessions
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ models/                  # Pydantic schemas (request/response + DB)
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ session.py
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ memory.py            # MemoryFragment, ConsentLevel enum, EmotionTag enum
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ emmy.py              # ConversationTurn, EmmyResponse
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ finetune.py          # TrainingExample JSONL schema
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€š
Ã¢â€â€š       Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ db/
Ã¢â€â€š       Ã¢â€â€š       Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ client.py            # asyncpg pool + Supabase service role client
Ã¢â€â€š       Ã¢â€â€š       Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ migrations/          # Raw SQL migrations (numbered, applied via supabase CLI)
Ã¢â€â€š       Ã¢â€â€š           Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ 001_subjects.sql
Ã¢â€â€š       Ã¢â€â€š           Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ 002_sessions.sql
Ã¢â€â€š       Ã¢â€â€š           Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ 003_memories.sql
Ã¢â€â€š       Ã¢â€â€š           Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ 004_emmy_profiles.sql
Ã¢â€â€š       Ã¢â€â€š           Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ 005_legacy_contacts.sql
Ã¢â€â€š       Ã¢â€â€š           Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ 006_rls_policies.sql
Ã¢â€â€š
Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ packages/
Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ shared-types/                    # Auto-generated from FastAPI OpenAPI spec
Ã¢â€â€š       Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ index.ts                     # Shared TS types consumed by web app
Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ scripts/
    Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ seed_demo.py                     # Populate demo subject with pre-built memory graph
    Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ build_training_data.py           # Standalone: generate JSONL from a subject's memories
    Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ export_emmy.py                   # Export all subject data as portable archive (GDPR)
```
...
#### `app/routers/emmy.py` Ã¢â‚¬â€ The Core Conversation Endpoint
...
#### `app/services/memory_extractor.py` Ã¢â‚¬â€ Structured Memory Extraction
...
#### `006_rls_policies.sql` Ã¢â‚¬â€ The Consent Enforcement Layer
...
### Phase 1 Ã¢â‚¬â€ Data Collection & Ingestion: The Interview Session Pipeline

**Step 1 Ã¢â‚¬â€ Subject Authentication & Session Initialization**
...
**Step 2 Ã¢â‚¬â€ WebSocket Connection to Gemini Live**
...
**Step 3 Ã¢â‚¬â€ Real-Time Audio Capture & Streaming**
...
**Step 4 Ã¢â‚¬â€ Authenticated Memory Capture**
...
**Step 5 Ã¢â‚¬â€ Session End & Post-Processing Trigger**
...
**Step 6 Ã¢â‚¬â€ Transcript Cleanup & Deep Memory Extraction**
...
**Step 7 Ã¢â‚¬â€ Embedding & Pinecone Upsert**
...
### Phase 2 Ã¢â‚¬â€ Backend & AI Logic: The Family Conversation Pipeline

**Step 1 Ã¢â‚¬â€ Persona Dataset: Building Evaluation Pairs**

Once a subject has completed 3+ sessions (~150+ memory fragments), the `finetune_builder.py` service generates a JSONL training file. Each training example is a `{"messages": [...]}` object where the system message sets the persona context, the user message is a plausible question a family member might ask, and the assistant message is a response written in the subject's voice Ã¢â‚¬â€ synthesized from their actual memory fragments, speech patterns extracted from transcripts (filler words, sentence rhythm, vocabulary level), and values identified across sessions.

Groq does not offer a hosted fine-tuning equivalent in this architecture. Emmy retains the consent-scoped JSONL as an evaluation/persona dataset and uses retrieval-grounded Groq generation instead of uploading private memories to a third-party fine-tuning job.

**Step 2 Ã¢â‚¬â€ Family Member Query Reception & Authorization**

A family member's question arrives at `POST /emmy/{emmy_id}/converse` as text or an audio blob. FastAPI validates the JWT, checks the `legacy_contacts` table to verify approved access, and determines the access level (`family` or `public`). Audio is transcribed through Groq before the pipeline continues.

**Step 3 Ã¢â‚¬â€ Consent-Aware Semantic Retrieval**
...
**Step 4 Ã¢â‚¬â€ Persona Model Prompt Construction**
...
3. A strict behavioral instruction: *"You may only draw on the memories provided above. If the question cannot be answered from these memories, say so warmly and specifically Ã¢â‚¬â€ do not invent details. Never speculate about events not captured in your memories."*

The Groq persona model then generates a response that sounds like the subject while being constrained to factual recall.

**Step 5 Ã¢â‚¬â€ Streaming Persona Response**
...
The goal of a hackathon demo is not to show features Ã¢â‚¬â€ it is to create an emotional moment that judges remember when they are deliberating at midnight. Every technical decision below is in service of producing that moment.

#### Wow Factor 01 Ã¢â‚¬â€ The Live Interview Demo
...
#### Wow Factor 02 Ã¢â‚¬â€ The Memory Graph Reveal
**A visual constellation of a person's inner life**

After the session, cut to the memory graph: 40+ nodes forming a glowing constellation Ã¢â‚¬â€ clustered by family, career, values, and place. Each node pulses gently. The presenter zooms into one Ã¢â‚¬â€ "The summer she almost didn't go to college" Ã¢â‚¬â€ and reads the memory fragment aloud. Then they toggle its consent level from Private to Family. Judges understand immediately: this is not a database. This is a mind, made navigable. The D3 force graph animation alone takes 3 seconds to render and generates an audible reaction in every test audience.

#### Wow Factor 03 Ã¢â‚¬â€ The Family Conversation Moment
**Ask her a question she's no longer alive to answer**

This is the demo's emotional climax. The presenter Ã¢â‚¬â€ playing the role of a granddaughter Ã¢â‚¬â€ speaks into the mic: *"Grandma, did you ever regret not traveling more?"* After 2 seconds, Emmy responds in a warm voice, drawing on a memory fragment from the session: a story about a trip to Portugal she almost took in 1974. The response is in her rhythm, her vocabulary. Below it, a "Sources" accordion expands showing the exact memory fragment and the session timestamp. The judge does not see a chatbot. They see a grandmother answering from across time. The demo ends there. No features list. No roadmap slide. Just silence.

#### Wow Factor 04 Ã¢â‚¬â€ The Ethical Boundary, Live
**Ask something Emmy doesn't know Ã¢â‚¬â€ and watch it refuse to fabricate**

Immediately after the emotional moment, the presenter asks: *"Grandma, what do you think about my husband?"* Ã¢â‚¬â€ a question the subject never addressed. Emmy responds: *"I don't have a memory of your husband Ã¢â‚¬â€ I wish I could tell you. But I'd love to hear about him from you."* This single moment differentiates Emmy from every ghost AI competitor and pre-empts the judges' ethical concern before they can raise it. The technical constraint becomes the emotional feature.

#### Wow Factor 05 Ã¢â‚¬â€ The Pre-Seeded Demo Fallback
**A fully populated demo subject ensures the demo never fails**

The `seed_demo.py` script populates a demo subject ("Eleanor, 74") with 87 pre-built memory fragments spanning 6 sessions, a completed fine-tune job, and a full memory graph. If any live component fails during the demo, the presenter switches to Eleanor's pre-built Emmy in 10 seconds. The family conversation still works identically. Judges never see a loading spinner. This is the most important engineering decision for a live demo under pressure.
...
| Time | Beat | Action |
|------|------|--------|
| 0:00 Ã¢â‚¬â€œ 0:20 | **The hook** | *"67% of adults say they regret not asking a deceased parent more questions. Emmy solves this Ã¢â‚¬â€ before it's too late."* |
| 0:20 Ã¢â‚¬â€œ 1:15 | **Live interview** | Show Gemini Live conducting a real 55-second interview with transcript streaming. |
| 1:15 Ã¢â‚¬â€œ 1:40 | **Memory graph** | *"Here's what 3 sessions produces."* Show the D3 constellation. Click one node. Read the memory. Show consent toggle. |
| 1:40 Ã¢â‚¬â€œ 2:30 | **The family conversation** | Ask the emotional question. Play Emmy's response with voice. Expand the sources accordion. Let the silence sit. |
| 2:30 Ã¢â‚¬â€œ 2:50 | **The ethical boundary** | Ask the unknowable question. Show Emmy's graceful refusal. *"This is what makes Emmy trustworthy, not just powerful."* |
| 2:50 Ã¢â‚¬â€œ 3:00 | **Close** | *"Emmy is live at emmy.app. The person you're thinking of right now Ã¢â‚¬â€ there's still time."* Cut. |

---

## 6. Personalised Voice Fingerprint Engine (Future Roadmap)

> **The Vision:** Emmy listens to how a person speaks Ã¢â‚¬â€ their pitch, bass, treble, loudness, depth, tone, rhythm, pauses, and the words they reach for most Ã¢â‚¬â€ and synthesises every future response in that exact acoustic identity. Not a generic voice preset. Not a rough clone. A mathematically precise replica of how someone sounds when they are most themselves.

---

### 6.1 The Six Acoustic Dimensions Emmy Will Capture
...
|---|-----------|------|-------------|
| Ã°Å¸Å½Å¡Ã¯Â¸Â | **Spectral Profile** | librosa Ã‚Â· MFCCs Ã‚Â· Mel filterbank | The physical shape of the voice Ã¢â‚¬â€ bass resonance, treble brightness, mid-range warmth. Captured as 40-coefficient MFCC vectors averaged across all session audio. This is the "colour" of someone's voice that makes them instantly recognisable. |
| Ã°Å¸Å½Âµ | **Pitch Contour** | CREPE Ã‚Â· PYIN Ã‚Â· F0 tracking | How pitch rises and falls across sentences Ã¢â‚¬â€ the melody of speech. Does the voice rise at the end of statements (uptalk)? Drop when being emphatic? CREPE tracks fundamental frequency (F0) frame-by-frame at 10ms resolution. |
| Ã°Å¸Â«Â | **Paralinguistic Texture** | Pyannote Ã‚Â· spectral flatness | Breathiness, vocal fry, nasality, and the sound of thinking Ã¢â‚¬â€ the micro-textures that make a voice feel human rather than synthesised. Captured via spectral flatness and harmonic-to-noise ratio analysis per utterance. |
| Ã¢ÂÂ±Ã¯Â¸Â | **Temporal Dynamics** | WebRTC VAD Ã‚Â· silence detection | Speech rate, pause duration before emotional responses, how long silences last when someone is thinking, acceleration when excited. These rhythms are as identifying as a fingerprint and almost entirely absent from current voice clones. |
| Ã°Å¸â€œÂ¢ | **Loudness & Dynamics** | RMS energy Ã‚Â· dynamic range | How loud someone naturally speaks, how much they vary their volume, whether they get quieter when saying something vulnerable or louder when telling a story. Captured as per-utterance RMS energy distributions with emotional context tags. |
| Ã°Å¸â€”Â£Ã¯Â¸Â | **Lexical & Prosodic Habits** | Groq Llama Ã‚Â· word frequency analysis | Which words are overused ("you know," "sort of," "exactly"), characteristic sentence openers, filler patterns, and where stress falls Ã¢â‚¬â€ the verbal fingerprint that exists in the language model layer but must sync with the audio layer to feel real. |

---

### 6.2 The Technical Pipeline Ã¢â‚¬â€ End to End

```
1. Audio Accumulation      Ã¢â€ â€™  Sessions stored in Supabase Storage
2. Speaker Diarization     Ã¢â€ â€™  Pyannote isolates subject-only audio segments
3. Feature Extraction      Ã¢â€ â€™  librosa + CREPE Ã¢â€ â€™ voice profile JSON
4. Profile Aggregation     Ã¢â€ â€™  Average across sessions Ã¢â€ â€™ stable fingerprint
5. Voice Model Training    Ã¢â€ â€™  ElevenLabs Pro Clone Ã¢â€ â€™ conditioned TTS
6. Conditioned Synthesis   Ã¢â€ â€™  Persona model text + voice fingerprint Ã¢â€ â€™ audio
```
...
**Step 1 Ã¢â‚¬â€ Diarization: Isolating the Subject's Voice**
...
- F0 standard deviation (how much they vary pitch Ã¢â‚¬â€ monotone vs. expressive)
...
**Step 4 Ã¢â‚¬â€ Temporal & Dynamic Analysis**

WebRTC VAD (Voice Activity Detection) timestamps are used to compute speech rate (syllables per second, estimated via zero-crossing rate), mean pause duration, and pause frequency. RMS energy analysis produces per-utterance loudness distributions. Crucially, utterances are tagged with their emotional context from the memory extraction pipeline Ã¢â‚¬â€ allowing the system to learn that this subject gets *quieter* when discussing grief and *faster* when excited, and to reproduce that emotional modulation during synthesis.

**Step 5 Ã¢â‚¬â€ ElevenLabs Professional Voice Clone Training**

Once 30+ minutes of clean subject audio is accumulated (typically after 2Ã¢â‚¬â€œ3 sessions), it is submitted to ElevenLabs' Professional Voice Clone endpoint (`POST /v1/voices/add` with `files[]` containing the cleaned audio segments). The Professional Clone Ã¢â‚¬â€ distinct from the Instant Clone Ã¢â‚¬â€ trains a speaker-conditioned neural TTS model that captures not just the spectral shape but also prosodic tendencies and vocal texture. The resulting `voice_id` is stored in `emmy_profiles.elevenlabs_voice_id`. All future TTS synthesis for this subject routes through this voice ID rather than a generic preset.

**Step 6 Ã¢â‚¬â€ Emotion-Conditioned Synthesis at Response Time**
...
- `stability` Ã¢â‚¬â€ inverse of the subject's measured F0 standard deviation (more variable speakers get lower stability)
- `similarity_boost` Ã¢â‚¬â€ set to 0.85+ for the professional clone
- `style` Ã¢â‚¬â€ derived from the emotional tag on the retrieved memory (grief responses get higher style for slower delivery, humor responses get lower for faster)

The result is not just the right voice Ã¢â‚¬â€ it is the right voice in the right emotional register for what is being said.

**Step 7 Ã¢â‚¬â€ Phase 3: Full Neural Conditioning (12Ã¢â‚¬â€œ18 Month Target)**
...
|-------|----------|----------|------|
| **Phase 1 Ã¢â‚¬â€ Live Interview Voice** | Available at launch | Gemini Live provides native session audio through a constrained ephemeral token. No audio analysis is required. | Provider free/developer tier subject to quota |
| **Phase 2 Ã¢â‚¬â€ ElevenLabs Professional Voice Clone** | 3Ã¢â‚¬â€œ6 months | Spectral + prosodic fingerprint extraction via librosa + CREPE. ElevenLabs Pro Clone trained on clean session audio. Emotion-conditioned synthesis settings per response. Subject must provide explicit voice clone consent separate from memory consent. | ~$5/subject training cost (after 30+ min of audio) |
| **Phase 3 Ã¢â‚¬â€ Full Acoustic Fingerprint Neural TTS** | 12Ã¢â‚¬â€œ18 months | All 6 acoustic dimensions captured and stored as fingerprint vector. VALL-E / VoiceBox-style neural codec model conditioned on fingerprint. Emotional register modulation. Characteristic filler words and hesitation sounds reproduced. Full local model Ã¢â‚¬â€ no third-party voice API dependency. | After 90+ min of audio |
...
Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ voice_fingerprint.py       # Core: librosa MFCC extraction + CREPE F0 tracking
Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ diarization_service.py     # Pyannote: isolate subject audio from AI audio
Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ voice_clone_service.py     # ElevenLabs Pro Clone: train, store voice_id, synthesise
Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ emotion_tts_mapper.py      # Map emotion tags Ã¢â€ â€™ ElevenLabs stability/style params
Ã¢â€â€š
Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ services/
Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ build_voice_profile.py     # Runs during session processing and updates the fingerprint
Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ db/migrations/
    Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ 007_voice_profiles.sql     # mfcc_vector, f0_mean, f0_std, speech_rate, pause_duration
    Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ 008_voice_consent.sql      # Separate consent flag: voice_clone_consented (boolean)

apps/web/components/
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ voice/
    Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ VoiceConsentModal.tsx      # Explicit, separate consent UI for voice cloning
    Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ VoiceProfileCard.tsx       # Subject dashboard: show voice fingerprint dimensions
    Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ VoicePreview.tsx           # Let subject hear a sample in their cloned voice before approving
```
...
| Spectral Analysis | librosa 0.10 | The canonical Python audio analysis library. Provides MFCC extraction, spectral centroid, zero-crossing rate, RMS energy, and harmonic-percussive separation in a single `pip install`. No GPU required Ã¢â‚¬â€ runs efficiently on the backend during post-session processing. |
...
| Emotion Mapping | Custom `emotion_tts_mapper.py` | A lightweight lookup service that maps the emotion tags from retrieved memory fragments to ElevenLabs voice parameter ranges. Grief Ã¢â€ â€™ stability 0.8, style 0.3 (slower, quieter). Humor Ã¢â€ â€™ stability 0.4, style 0.7 (faster, more variable). Pride Ã¢â€ â€™ stability 0.6, style 0.5. These ranges are seeded with defaults and refined per-subject based on family feedback. |

---

### 6.7 The Consent Architecture for Voice Cloning

> **Critical ethical distinction:** Voice cloning consent is handled entirely separately from memory consent. A subject may share all their memories with family but explicitly opt out of having their voice cloned. In this case, Emmy uses the Phase 1 voice preset for all synthesis. The voice clone is never activated without an explicit, standalone consent action Ã¢â‚¬â€ a separate modal with plain-language explanation, a typed confirmation, and a recorded timestamp stored in `voice_profiles.clone_consented_at`.

**Subject consent at activation:** After 30+ minutes of audio is accumulated, the subject receives a notification: *"Emmy can now learn your voice. This means future responses will sound more like you. You can turn this off at any time."* They must tap "I consent to voice learning" in a dedicated modal Ã¢â‚¬â€ not a checkbox buried in settings.
...
**Family member disclosure:** Every Emmy conversation page shows a persistent label: *"Responses synthesised by AI in Eleanor's voice Ã¢â‚¬â€ not original recordings."* Family members cannot be confused about whether they are hearing a real recording or a synthesis.
...
```
This makes it structurally impossible to store a voice ID without a consent timestamp Ã¢â‚¬â€ the consent is not just a UI gate, it is a database-level invariant.

---

*Emmy Ã¢â‚¬â€ Full Product Blueprint Ã‚Â· Built for OpenAI Hackathon Ã‚Â· All systems: Next.js 14 Ã‚Â· FastAPI Ã‚Â· Supabase Ã‚Â· Pinecone Ã‚Â· Gemini Live Ã‚Â· Groq Llama Ã‚Â· Gemini embeddings Ã‚Â· librosa Ã‚Â· CREPE Ã‚Â· ElevenLabs Pro Clone Ã‚Â· Pyannote*

---

## 7. Deployment Instructions

Emmy is designed to be deployed with zero code changes using Railway for the backend and Vercel for the frontend.
...
3. Configure the following environment variables in Railway:
   ...
   - `CORS_ORIGINS` (set this to the exact Vercel browser origin, with no path; for the current deployment: `https://emmy-web-mocha.vercel.app`). Multiple approved origins can be comma-separated. Do not use `*`, because authenticated requests require credentialed CORS.
...
3. Configure the following environment variable in Vercel:
   - `NEXT_PUBLIC_API_BASE_URL` (Set to your Railway backend URL, e.g., `https://emmy-api.up.railway.app`)
