...
async def _resolve_access(conn: asyncpg.Connection, caller_id: str, requested_subject_id: str | None) -> tuple[str, str, str]:
    """Return subject id, display name, and the caller's consent scope."""
    subject_id = requested_subject_id or caller_id
    subject = await conn.fetchrow("SELECT id, user_id, full_name FROM subjects WHERE id = $1", subject_id)
    if not subject:
        if requested_subject_id is None or subject_id == caller_id:
            return caller_id, "Your legacy", "owner"
        raise HTTPException(status_code=404, detail="This Echo legacy was not found")

    owner_id = str(subject["user_id"])
    if owner_id == caller_id:
        return str(subject["id"]), subject["full_name"] or "Your legacy", "owner"
    ...
    return str(subject["id"]), subject["full_name"] or "Echo", str(invitation["access_level"])


async def _citations_for(conn: asyncpg.Connection, memories: list[dict[str, Any]]) -> list[Citation]:
...
async def conversation(
    ...
    subject_id, subject_name, access_level = await _resolve_access(conn, caller_id, payload.subject_id)
    ...
    allowed = ["private", "family", "legacy"] if access_level == "owner" else ["family", "legacy"]
    ...
    citations = await _citations_for(conn, [memory for memory in memories if str(memory.get("memory_id") or memory.get("id")) in set(plan.citations or plan.required_memories)])
    explainability = Explainability(
