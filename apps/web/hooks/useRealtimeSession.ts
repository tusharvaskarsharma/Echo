...
export function useRealtimeSession() {
  ...
  const cleanup = useCallback(() => {
    ...
    connectedRef.current = false; setIsConnected(false); setIsSpeaking(false);
  }, []);
  ...
  const playGeminiAudio = useCallback(async (base64: string) => {
    ...
    const source = context.createBufferSource(); source.buffer = audioBuffer; source.connect(context.destination);
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      const socket = new WebSocket(`${GEMINI_LIVE_URL}?access_token=${encodeURIComponent(tokenPayload.access_token)}`);
      ...
      socket.addEventListener("close", () => { console.info("[Gemini Live] WebSocket closed"); connectedRef.current = false; setIsConnected(false); setIsSpeaking(false); });
      ...
      const source = inputContext.createMediaStreamSource(stream); const processor = inputContext.createScriptProcessor(4096, 1, 1); const mutedGain = inputContext.createGain(); mutedGain.gain.value = 0;
      sourceRef.current = source; processorRef.current = processor; mutedGainRef.current = mutedGain;
      ...
      source.connect(processor); processor.connect(mutedGain); mutedGain.connect(inputContext.destination);
