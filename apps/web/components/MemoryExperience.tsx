...
export function MemoryExperience({ compact = false }: { compact?: boolean }) {
  ...
  return <>
    {!compact && <section className="subject-hero"><div><p className="eyebrow">Your living legacy</p><h1>{profile.name}</h1><p>{profile.bio}</p></div><div className="stat-stack"><strong>{profile.session_count}</strong><span>recorded sessions</span><strong>{memories.length}</strong><span>memories held with care</span></div></section>}
